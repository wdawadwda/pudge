...
import { addPhotoSchema } from "~/entities/const/validation";
...
import { sendGalleryData } from "~/store/api/galleryApi";

import { type FormField } from "./addPhoto.type";
import { addPhotoFormSchema } from "./formSchema";
...
export const AddPhoto = () => {
  ...
  const onClick = () => {
    ...
    if (photoFiles instanceof FileList) {
      ...
      formData.append("text", getValues("text").toLowerCase());
      formData.append("img", photoFiles[0]);
    }
    sendGalleryData(formData)
      .then((responseData) => {
  ...
  return (
    ...
    >
      ...
      <form>
        ...
        {addPhotoFormSchema.map((field: FormField) => (
          <div key={field.name}>
            <div>
              <label
                className={
